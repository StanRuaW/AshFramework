...
namespace AshFramework.Fsm
{
    ...
    internal sealed class Fsm<T> : FsmBase, IFsm<T> where T : class
    {
        ...
        public static Fsm<T> Create(string name, T owner, params FsmState<T>[] states)
        {
            ...
            {
				Log.L("FSM states is invalid.");
                //throw new GameFrameworkException("FSM states is invalid.");
            }

			//TODO:引入状态机
			Fsm<T> fsm = new Fsm<T>();//ReferencePool.Acquire<Fsm<T>>();
            fsm.Name = name;
            fsm.m_Owner = owner;
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                {
					Log.L(string.Format("FSM '{0}' state '{1}' is already exist.", new TypeNamePair(typeof(T), name), stateType.FullName));
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            if (IsRunning)
            {
				Log.L("FSM is running, can not start again.");
            ...
            if (state == null)
            {
				Log.L(string.Format("FSM '{0}' can not start state '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), typeof(TState).FullName));
        ...
        public void Start(Type stateType)
        {
            ...
            if (stateType == null)
            {
				Log.L("State type is invalid");
				//throw new GameFrameworkException("State type is invalid.");
            }

            if (!typeof(FsmState<T>).IsAssignableFrom(stateType))
            {
				Log.L(string.Format("State type '{0}' is invalid.", stateType.FullName));
            ...
            if (state == null)
            {
				Log.L(string.Format("FSM '{0}' can not start state '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), stateType.FullName));
        ...
        internal void ChangeState(Type stateType)
        {
            if (m_CurrentState == null)
            {
				Log.L("Current state is invalid.");
            ...
            if (state == null)
            {
				Log.L(string.Format("FSM '{0}' can not change state to '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), stateType.FullName));
